...
fn main() {
    ...
    for (name, value) in &table {
        ...
        let default_value = entry
            .get("default")
            .unwrap_or_else(|| panic!("setting `{name}` has no default"));
        let (default_expr, default_display) = match ty.as_str() {
            "list<string>" => {
                let items = strings_at(entry, name, "default");
                (vec_expr(&items), toml_list(&items))
            }
            "float" => {
                let v = default_value
                    .as_float()
            ...
            "bool" => {
                let v = default_value
                    .as_bool()
                    .unwrap_or_else(|| panic!("setting `{name}`: default must be true or false"));
                (format!("{v}"), format!("{v}"))
            }
            other => panic!("no default handling for `{other}`"),
        };
        ...
        writeln!(fields, "    pub {name}: {},", rust_type(&ty)).unwrap();
        writeln!(defaults, "            {name}: {default_expr},").unwrap();
        writeln!(
            metas,
            "    SettingMeta {{\n        \
             name: {name:?},\n        \
