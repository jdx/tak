...
src/machine.rs     the OS, CPU and memory an export was measured on; never recorded
...
generated by Communique. Write them for a tak user who has not read the diff
...
For a hypothetical fix, prefer `fix(history): separate results from different runners`
over `fix: address review feedback`. Its description should show the history command and explain which measurements are comparable; preserve the pre-v1 warning and distinguish instruction counts from timing.
