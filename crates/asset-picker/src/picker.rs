//! Platform detection and release-asset selection.
//!
//! Lifted from mise's `src/backend/asset_matcher.rs` (MIT, Copyright (c) 2025
//! Jeff Dickey) — see the crate root for the notice. Only the pure scoring core
//! is here; mise's checksum fetching needs an HTTP client and is out of scope.
//!
//! The regex tables are the valuable part. They encode years of accumulated
//! knowledge about how projects actually name their release assets, which is
//! not something worth rediscovering one wrong measurement at a time.

use regex::Regex;
use std::sync::LazyLock;

use crate::format::Format;
use crate::tokens::is_platform_or_version_token;

/// The last path segment of a URL, or the whole string if it is not a URL.
///
/// mise uses the `url` crate here; a rsplit is enough for the one thing this is
/// used for — recovering an asset filename in order to pattern-match it — and
/// keeps the dependency list to `regex` alone.
fn get_filename_from_url(url_str: &str) -> String {
    let path = url_str
        .split_once("://")
        .map_or(url_str, |(_, rest)| rest)
        .split(['?', '#'])
        .next()
        .unwrap_or(url_str);
    path.rsplit('/').next().unwrap_or(url_str).to_string()
}
...
impl AssetPicker {
    ...
    fn score_format_preferences(&self, asset: &str) -> i32 {
        let format = Format::from_file_name(asset);
...
mod tests {
    ...
    /// Real assets from `oxc-project/oxc`, which ships two binaries per
    /// platform — the case that makes name-based disambiguation necessary.
